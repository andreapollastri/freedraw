...
class SvgPathRecorder implements PathSink {
  ...
  ellipse(
    ...
    const segments = Math.max(
      2,
      Math.ceil((Math.abs(endAngle - startAngle) / Math.PI) * 4),
    );
    ...
    for (let i = 0; i < segments; i += 1) {
      ...
      const c1 = {
        x: p0.x - kappa * rx * Math.sin(a0),
        y: p0.y + kappa * ry * Math.cos(a0),
      };
      const c2 = {
        x: p1.x + kappa * rx * Math.sin(a1),
        y: p1.y - kappa * ry * Math.cos(a1),
      };
