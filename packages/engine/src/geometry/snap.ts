...
function nearestSurfacePoint(element: Element, world: Point): Point {
  ...
  const surface = outline
    ? closestPointOnOutline(local, outline, bounds)
    : closestPointOnRect(local, bounds);
...
function closestPointOnOutline(
  ...
  if (outline.kind === "polygon")
    return closestPointOnPolygon(point, outline.points);
...
function closestPointOnEllipse(
  point: Point,
  ellipse: Extract<Outline, { kind: "ellipse" }>,
): Point {
  ...
  if (dx === 0 && dy === 0)
    return { x: ellipse.cx, y: ellipse.cy - ellipse.ry };
  const scale =
    1 /
    Math.sqrt(
...
function closestPointOnRect(
  point: Point,
  rect: ReturnType<typeof elementBounds>,
): Point {
...
export function snapEndpoint(
  ...
  const shapeSnap = snapToShapes(
    world,
    snapshot,
    options.threshold * EDGE_CENTER_MAGNET_FACTOR,
    options.ignoreId,
  );
  if (shapeSnap.target) {
    return {
      point: shapeSnap.point,
      target: shapeSnap.target,
      guides: [{ kind: "point", at: shapeSnap.point }],
    };
  }
  const surfaceSnap = snapToShapeSurface(
    world,
    snapshot,
    options.threshold,
    options.ignoreId,
  ...
  if (surfaceSnap.target) {
    return {
      point: surfaceSnap.point,
      target: surfaceSnap.target,
      guides: [{ kind: "point", at: surfaceSnap.point }],
    };
  }
  ...
  if (options.origin)
    return {
      point: snapToGridAxis(world, options.origin, options.threshold),
      target: null,
      guides: [],
    };
...
function snapToGridAxis(world: Point, origin: Point, threshold: number): Point {
  ...
  if (dy <= axisThreshold && dy * 2 <= dx)
    return { x: snapValueToGrid(world.x), y: origin.y };
  if (dx <= axisThreshold && dx * 2 <= dy)
    return { x: origin.x, y: snapValueToGrid(world.y) };
